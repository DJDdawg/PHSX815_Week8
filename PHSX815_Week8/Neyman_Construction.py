#colloborated w/ Neema 

#import packages
import numpy as np
import matplotlib.pyplot as plt
from scipy.stats import norm
from matplotlib.colors import LinearSegmentedColormap #to change the color of the heat map
import sys

#Define simple model with a Single Parameter: Coin Flip
def coin_flip(p, n):
    Heads = np.sum(np.random.binomial(1, p, size=n)) #Number of heads when a coin with probability p is flipped n times
    return Heads 

if __name__ == "__main__":
   
    #Initialize Values 
    true_p_values = np.linspace(0, 1, num=100) #100 values for p between 0 and 1
    Nmeas = 10
    Nexp = 100
    ...
    #Simulate experiments for all true p values
    observed_counts = []
    ...
    colors = [(0, 0, 1), (1, 0.5, 1), (1, 0, 0)] #blue, , red
    cmap = LinearSegmentedColormap.from_list('custom', colors)

    # Neyman Plot and 2D heatmap 
    ...
    ax1.set_title('Neyman Plot for Coin Flip Experiment')
